...
!----------------------------------------------------------------------------------------------------
!The initial condition is an analytical solution for the Pringle-Model with beta-viscosity,
!here dimensionless quantities are used, M_Z = 1/G, cs = 1/mach, ...
...
!----------physical parameters-----------
 ...
 log_mass_central   =  LOG10(1.0d0/c_grav)
 log_mass_disk      =  8.0d0
 beta               =  0.01d0
 cs                 =  0.005d0          !=1/mach
 accr_eff           =  0.1d0
...
!----------store options-----------------
 ...
 time_store         =  1.D+5                                !store after this time
...
CASE (3)
   ...
   sigma(1:N) = (3.0d0/(SQRT(tau0)*((4.0d0*c_pi)**(3.0d0/2.0d0)))) * (radius(1:N))**(-9.0d0/8.0d0) &
                * EXP( - (1.0d0 - radius(1:N)**(3.0d0/4.0d0) )**2/tau0 ) + 1.E-10


   u(1,1:N) = sigma(1:N)*radius(1:N)
   u(3,1:N) = u(1,1:N)*SQRT(c_grav*mass_central*radius(1:N))
   u(2,1:N) = u(1,1:N) * (9.0d0/2.0d0) * beta * (radius(1:N)/tau0) * (1.0d0 - (radius(1:N))**(-3.0d0/4.0d0))
