...
!----------------------------------------------------------------------------------------------------
!comparison of the Pringle-Model with an analytical solution (Pringle-disk)


!----------physical parameters-----------

 disk_inner_radius  =  1.D-3
 disk_outer_radius  =  100.0d0
 ...
 mode_viscosity     =  2                                    !1: beta-viscosity, 2: Pringle-viscosity
 mode_initial_cond  =  3
...
!----------numerical parameters----------
 ...
 mode_boundarys     =  5                                    !1: non-reflecting, 2: reflecting, 3: no-gradients, 4: simple model, 5: pringle-disk, 6: set by initial conditions
...
CASE (3)
   ...
   DO i = 1, N
     xi = SQRT(radius(i)/s_0)
     sigma(i) = (sigma_0/(TWO*(xi**3)*SQRT(c_pi*tau))) * (EXP(-(xi-1)**2/(FOUR*tau)) - EXP(-(xi+1)**2/(FOUR*tau)))
   END DO
